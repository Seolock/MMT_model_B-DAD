...
class Recorder():
    ...
    def extract_full_map(self):
        if self.steps:
            full_map = torch.cat(self.steps, dim=0)
            print(full_map.shape)
            self.steps = []
            # return full_map
            torch.save(full_map, os.path.join(self.map_path, str(self.n)+'map.pth'), _use_new_zipfile_serialization=False)
            self.n += 1
